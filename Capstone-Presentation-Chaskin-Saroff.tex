...
\title[Recovering the Metric - An Overview of Lie Agebra's]{Recovering the Metric - An Overview of Lie Agebra's}
\author[C.\, Saroff]{Chaskin Saroff}
\institute[SUNY Oswego]{Oswego State University\\ Department of Mathematics}
\date[16 Sep 2014]{16 September 2014}
...
Let $V$ be any vector space, and define
$$
...
The basis vectors $x, y$, and $z$ can be represented by the following matrices
$$
x = \begin{pmatrix}
1 & \red{1} & 0 \\
0 & 1 & 0 \\
...
$$
\pause
$\bullet\ \lag{h}_3$ is \emph{not} a subalgebra of $\lag{gl}(3,\R)$ because it doesn't have
the same bracket.
\end{frame}
...
{\bf Example} The \at{dot product} on $\R^n$:
\pause
Let $x, y \in \R^n$; then $x \cdot y = x^Ty$.
\pause

\

{\bf Example} The \at{Minkowski inner product} on $\R^n$:
\pause
Let $x = (t,x_1,x_2,x_3)$ and 
$y = (s, y_1, y_2, y_3)$ be two vectors in $\R^4$.
\pause
Then the Minkowski inner product of
$x$ and $y$ is given by
$$
\inp{x}{y} = -(ts) + x_1y_1 + x_2y_2 + x_3y_3.
$$
\pause

This is the inner product that gives rise to Einstein's Special Relativity!
\end{frame}
...
\begin{frame}{Lie Algebra Seminar}
There is a ``Group Study Seminar'' on Lie Algebras that meets every week!
...
{\bf Time:} 5:30 -- 6:30 pm

\smallskip

{\bf Room:} Math Commons
\end{center}

\

We'll start with a ``review'' of vector spaces, then spend a lot of time building
examples. Eventually we'll prove some theorems, and study a little geometry
(time permitting).
\end{frame}
