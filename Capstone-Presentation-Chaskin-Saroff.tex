...
\begin{frame}{Lie Bracket as Linear Combination of Bases}
    ...
    %things too beamer''
    \uncover<00->{We can write $\br{x}{y}$ as a linear combination their component vectors's
brackets.}
    \uncover<01->{\\Let's look at an example of a 2 dimensional space}
    \uncover<02->{\\Let $x,y \in \fg$ with}
    \uncover<03->{\\$x=w_1e_1 + w_2e_2$}
    \uncover<04->{\\$y=v_1e_1 + v_2e_2$}
    \begin{align*}
        \uncover<05->{\text{So} \br{x}{y} &= \br{w_1e_1 + w_2e_2}{v_1e_1 + v_2e_2}}
        \uncover<06->{\\&= \br{w_1e_1}{v_1e_1 + v_2e_2} + \br{w_2e_2}{v_1e_1 + v_2e_2}}
        \uncover<07->{\\&= \br{w_1e_1}{v_1e_1} + \br{w_1e_1}{v_2e_2}+ \br{w_2e_2}{v_1e_1} + \br{w_2e_2}{v_2e_2}}
        \uncover<08->{\\&= w_1v_1\br{e_1}{e_1} + w_1v_2\br{e_1}{e_2}+ w_2v_1\br{e_2}{e_1} + w_2v_2\br{e_2}{e_2}}
        \uncover<09->{\\&= w_1v_1L_{11} + w_1v_2L_{12} + w_2v_1L_{21}+ w_2v_2L_{22}}
    \end{align*}
    \uncover<10->{This means that the Lie Bracket is fully described by the matrix, L}
\end{frame}
\begin{frame}{Generalizing the Lie Bracket}
    ...
    \end{pmatrix}
     ...
     =
     \begin{pmatrix}
         w_1L_{11} + w_2L_{12}, w_1L_{12}+w_2L_{22}
     \end{pmatrix}
    $$
    \pause
    $$
    ...
    $$
    =   \begin{pmatrix}
            w_1v_1L_{11} + w_2v_1L_{12} + w_1v_2L_{12}+w_2v_2L_{22}
        \end{pmatrix}
    $$
    \pause
    That is
...
\begin{frame}{$\h_3$'s Lie Bracket}
    ...
    \pause
    \\$\h_3$ is two-step nilpotent
    \pause
    \\$\br{\br{x}{y}}{z} \ \forall x,y,z \in \h_3$
    \pause
    $$
    \begin{pmatrix}
          \zvec & e_3   & \zvec
...
\begin{frame}{Inner Product as a Matrix}
    ...
    $$
        E = \begin{pmatrix}
            E_{11} & \tikzmark{left}E_{12}\tikzmark{right} & \cdots & E_{1n} \\
            \DrawBox[thick]
            \tikzmark{left}E_{12}\tikzmark{right} & E_{22} & \cdots & E_{2n} \\
                \vdots & \vdots & \ddots & \vdots \\
                E_{1n} & E_{1 n-1} & \cdots & E_{nn}
            \end{pmatrix}
            \DrawBox[thick]
    $$
\end{frame}
...
\section*{TikZ}
\subsection*{}
\begin{frame}{The Lie Map}
\small
The Lie Bracket is a matrix of vectors that can be visually represented. First, let's lay our matrix, $L$, down as if it were on a table.\\
\pause
            \includegraphics[width=20mm]{Lie_MX_Flat.tikz} %input file
            \linebreak
\pause
            Now, let's pick out a single $L_{i, j}$ from that matrix.\\
            \includegraphics[width=20mm]{Lie_MX_Flat_ij.tikz} %input file
            \linebreak
\pause
            This is what is really lying behind that element!\\
            \includegraphics[width=20mm]{Lie_MX_Vector.tikz} %input file
\normalsize

\end{frame}

\begin{frame}
Now that we have this visualization of what an element of the $L$ mapping is, we can imagine how the whole matrix appears.\\
            \begin{center}
            \includegraphics[width=20mm]{Lie_MX_Flat.tikz}
            \includegraphics[width=20mm]{Lie_MX_Flat_ij.tikz}
            \includegraphics[width=20mm]{Lie_MX_Vector.tikz}
            \end{center}
            \pause
% my crude attempt to draw a stack of matrices using picture environment
