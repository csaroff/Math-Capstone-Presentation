...
\title[Encoding the Bracket - An Overview of Lie Agebra's]{Encoding the Bracket - An Overview of Lie Algebras}
...
$$
\br{x}{y} = 0 \text{\ \ for\ all\ } x,y \in \fg.
$$
\pause
...
bilinear, postive definite function $\inpe :\fg \times \fg \to \R$.
\pause
\begin{enumerate}
\item If $\inp{x}{y} = 0$ for all $y \in \fg$, then $x$ must be $0$;
\pause
...
\begin{frame}{$j$-maps}
    In order to seperate the interaction between the center and the non-center
...
\begin{frame}{The Lie Bracket as a Matrix with Vector entries}
    ...
    $$
    \pause
    We will denote the $k$th entry of $L_{ij}$ as $L_{ij}^k$.
\pause
\\$[e_i,e_i] = \zvec$
\end{frame}
\begin{frame}{The Lie Bracket as a Matrix with Vector entries}
...
\begin{frame}
    ...
    \end{pmatrix}.
    $$
    These $L_{ij}$ can then be arranged in an $n \times n$ matrix
    $$
    L = (L_{ij}) = \begin{pmatrix}
        L_{11} & L_{12} & \cdots & L_{1n} \\
        L_{21} & L_{22} & \cdots & L_{2n} \\
...
\begin{frame}{Lie Bracket as Linear Combination of Bases}
    ...
    %things too beamer''
    \uncover<00->{We can write $\br{x}{y}$ as a linear combination of their component vectors'
brackets.}
...
\begin{frame}{Inner Product as a Matrix}
    ...
    \pause
    \\Postive Definite: $\inp{x}{x} \ge 0 \ \text{and} \ \inp{x}{x}=0 \implies x=\zvec$
    \\And $\zvec$ canot be a basis vector because it is linearly dependent with all vectors.
    \\So $\blu{e_{ii}>0}$
\end{frame}
\begin{frame}{Inner Product as a Matrix}
    The Inner Product can be represented by a matrix very similarly to the Lie Bracket.
...
\begin{frame}{What we did}
    Using the programming language, sage, we created a program that would
    compute the $j$-maps of an arbitrary Lie algebra.
